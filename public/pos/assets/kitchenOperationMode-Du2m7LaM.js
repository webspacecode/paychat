import{g as ot,b as h}from"./index-D8qREOz2.js";const zt="/color-paychat-logo-main.svg",Mt="\x1BE",Dt="\x1BE\0",Rt="\x1BG",Ut="\x1BG\0",Bt=8,ct={"58mm":{width:"58mm",columns:32,fontSize:"10px",titleSize:"13px",logoMaxWidth:"136px",paychatLogoWidth:"72px",qrSize:"86px",padding:"6px"},"80mm":{width:"80mm",columns:48,fontSize:"12px",titleSize:"16px",logoMaxWidth:"210px",paychatLogoWidth:"96px",qrSize:"112px",padding:"8px"}},mt=(t,e={})=>{if(!t||t==="null")return e;try{return JSON.parse(t)}catch{return e}},st=t=>{try{return typeof localStorage>"u"?"":localStorage.getItem(t)||""}catch{return""}},Kt=()=>mt(st("tenant_info"),{}),jt=()=>mt(st("selected_location"),{}),ht=t=>M(t).replace(/\s+-\s+/g," ").replace(/\s{2,}/g," ").trim(),gt=t=>ht(t).replace(/[-_]+/g," ").replace(/\s{2,}/g," ").trim().toLowerCase().replace(/\b[a-z]/g,e=>e.toUpperCase()),ft=t=>{const e=ht(t);if(!e)return"";const o=e.split(",").map(n=>n.trim()).filter(Boolean);return(o.length?o.slice(0,2).join(", "):e).slice(0,80)},c=t=>String(t??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;"),q=t=>c(t).replace(/`/g,"&#096;"),M=t=>String(t??"").replace(/[\u20b9]/g,"Rs.").replace(/[^\x20-\x7E\n]/g,"").trim(),u=t=>Number(t||0).toFixed(2),W=t=>{const e=Number(t||0);return Number.isInteger(e)?String(e):e.toFixed(2)},bt=(t=new Date)=>{const e=t?new Date(t):new Date;return Number.isNaN(e.getTime())?new Date().toLocaleString("en-IN"):e.toLocaleString("en-IN",{day:"2-digit",month:"short",year:"numeric",hour:"2-digit",minute:"2-digit"})},_t=(t="80mm")=>ct[t]||ct["80mm"],d=(...t)=>{for(const e of t)if(e!=null&&e!=="")return e;return""},Qt=t=>{const e=String(t||"").trim();if(!e)return"";try{const o=typeof window<"u"?window.location.origin:"https://paychat.local",s=new URL(e,o).pathname.split("/").map(a=>a.trim()).filter(Boolean),r=s.findIndex(a=>["invoice","invoices"].includes(a.toLowerCase())),i=r>=0?s[r+1]:s[s.length-1];return decodeURIComponent(i||"").trim()}catch{const s=e.split("?")[0].split("#")[0].split("/").map(r=>r.trim()).filter(Boolean);return s[s.length-1]||""}},L=(...t)=>{for(const e of t){if(e==null||e==="")continue;const o=Number(e);if(Number.isFinite(o))return o}return 0},Ft=(...t)=>{for(const e of t)if(Array.isArray(e)&&e.length)return e;return[]},Gt=(t={})=>!t||typeof t!="object"?!1:!!(t.product||t.menu_item||t.product_snapshot||t.product_name||t.item_name||t.menu_item_name||t.name||t.title||t.quantity||t.qty||t.price||t.rate||t.unit_price||t.total||t.line_total||t.amount),tt=(t,e=0,o=new Set)=>{if(!t||e>4)return[];if(Array.isArray(t))return t.some(Gt)?t:[];if(typeof t!="object"||o.has(t))return[];o.add(t);const n=["items","order_items","orderItems","line_items","lineItems","cart"];for(const s of n){const r=tt(t[s],e+1,o);if(r.length)return r}for(const s of Object.values(t)){const r=tt(s,e+1,o);if(r.length)return r}return[]},Wt=(t={})=>d(t.product?.name,t.menu_item?.name,t.product_snapshot?.name,t.product_name,t.item_name,t.menu_item_name,t.name,t.title,t.description,"Item"),rt=(t={})=>L(t.quantity,t.qty,t.pivot?.quantity,1)||1,yt=(t={})=>{const e=rt(t),o=d(t.rate,t.price,t.unit_price,t.unitPrice,t.product?.price);if(o!=="")return Number(o||0);const n=d(t.total,t.line_total,t.amount,t.subtotal);return Number(n||0)/e},Ht=(t={})=>{const e=d(t.total,t.line_total,t.amount,t.subtotal);return e!==""?Number(e||0):yt(t)*rt(t)},Jt=(t={})=>{const e=t.invoice||t.invoice_data||{},o=t.data||t.order||{};return Ft(t.items,t.order_items,t.orderItems,t.line_items,t.lineItems,t.cart,e.items,e.order_items,o.items,o.order_items,o.line_items,tt(t))},Vt=(t={})=>{const e=Array.isArray(t.payments)?t.payments[0]:null;return d(t.payment_method,t.payment_mode,t.payment?.method,e?.payment_method,e?.method)},lt=(...t)=>{const e=[];return t.flat().forEach(o=>{if(!o)return;if(typeof o=="string"||typeof o=="number"){e.push(String(o));return}const n=d(o.code,o.kot_code,o.batch_code,o.token_code,o.id);n&&e.push(String(n))}),[...new Set(e)]},Yt=t=>{let e=String(t||"").trim();if(!e)return"";if(e.startsWith('"')&&e.endsWith('"'))try{e=JSON.parse(e)}catch{}if(/&lt;\s*(?:svg|img)\b/i.test(e)&&(e=e.replace(/&lt;/gi,"<").replace(/&gt;/gi,">").replace(/&quot;/gi,'"').replace(/&#0?39;/gi,"'").replace(/&amp;/gi,"&")),!/<(?:svg|img)\b/i.test(e)&&/^[a-z0-9+/=\s]+$/i.test(e))try{const o=typeof atob=="function"?atob(e.replace(/\s+/g,"")):"";/<(?:svg|img)\b/i.test(o)&&(e=o)}catch{}return e.trim()},Xt=t=>{if(!t)return"";const e=Yt(t),o=e.match(/<svg\b[\s\S]*?<\/svg>/i);if(o){const s=`data:image/svg+xml;charset=utf-8,${encodeURIComponent(o[0])}`;return`<img class="qr-image" src="${q(s)}" alt="Invoice QR" />`}const n=e.match(/<img\b[^>]*\bsrc\s*=\s*["']([^"']+)["'][^>]*>/i);return n?.[1]?`<img class="qr-image" src="${q(n[1])}" alt="Invoice QR" />`:/^(data:image\/|https?:\/\/|\/)/i.test(e)?`<img class="qr-image" src="${q(e)}" alt="Invoice QR" />`:`<div class="qr-url">${c(e)}</div>`},Zt=(t={},e={})=>{if(!t||typeof t!="object")throw new Error("Order data is required");const o=Kt(),n=ot(),s=o?.tenant||{},r=o?.branding||s?.branding||o?.branching||{},i=jt(),a={...i&&typeof i=="object"?i:{},...t.location&&typeof t.location=="object"?t.location:{}};t.branch||t.branching||t.branding||o?.branch||o?.branching;const p=t.merchant||t.receipt?.merchant||{},l=t.invoice||t.invoice_data||t.receipt?.invoice||{},m=t.qr||t.receipt?.qr||{},_=d(e.invoiceUrl,t.invoice_url,t.invoiceUrl,l.url,t.meta?.invoice?.url,m.invoice_url),y=Jt(t).map(x=>({name:Wt(x),qty:rt(x),rate:yt(x),total:Ht(x)})),$=L(t.subtotal,t.totals?.subtotal,y.reduce((x,S)=>x+S.rate*S.qty,0)),A=L(t.discount?.amount,t.discount,t.discount_amount,t.totals?.discount,t.totals?.discount_total),k=L(t.tax,t.tax_amount,t.total_tax,t.totals?.tax,t.totals?.tax_total,t.totals?.total_tax,t.tax_summary?.total_tax),N=L(t.total,t.grand_total,t.totals?.grand_total,$+k-A);return{shopName:d(e.shopName,p.name,a.tenant?.name,t.tenant?.name,r.company_name,s.name,st("tenant_slug"),"PayChat POS"),shopPhone:d(e.shopPhone,p.phone,a.phone,r.phone,s.phone),shopAddress:d(e.shopAddress,r.address,s.branding?.address,t.tenant?.branding?.address,a.tenant?.branding?.address),shopLogoUrl:d(e.shopLogoUrl,a.logo,a.tenant?.logo,t.tenant?.logo,r.logo,s.logo),locationName:d(a.name,t.location_name),paychatLogoUrl:d(e.paychatLogoUrl,t.paychat_logo_url,zt),invoiceNo:d(e.invoiceNo,t.invoice_no,t.invoiceNo,l.number,l.invoice_no,l.invoiceNo,l.invoice_number,l.offline_invoice_number,t.meta?.invoice?.number,t.meta?.invoice?.invoice_no,t.meta?.invoice?.invoiceNo,t.meta?.invoice?.invoice_number,t.offline_invoice_number,t.local_invoice_no,Qt(_)),orderNo:d(t.order_no,t.orderNo,t.local_order_id,t.id),dateTime:d(t.created_at,t.completed_at,t.updated_at,t.offline_created_at,new Date),orderType:d(t.order_type,t.orderType,t.delivery_channel_label,t.delivery_channel),tableName:d(t.table_display,t.tableDisplay,t.table_session?.table_display,t.tableSession?.tableDisplay,t.table_session?.table?.name,t.tableSession?.table?.name,t.table?.name,t.table?.code,t.table_name,t.table_no,t.table_number),guestCount:d(t.guest_count,t.guestCount,t.table_session?.guest_count),tokenNo:d(t.token?.token_code,t.token_code,t.token?.offline_token_number,t.offline_token_number),kotCodes:lt(t.kot_codes,t.kotTokens,t.kot_tokens,t.kitchen_batches,t.batches),batchCodes:lt(t.batch_codes,t.batchCodes),items:y,subtotal:$,discount:A,tax:k,grandTotal:N,paidAmount:L(t.paid_amount,t.payment?.amount,t.totals?.paid_amount,N),paymentMethod:Vt(t),invoiceUrl:_,invoiceQr:d(e.invoiceQr,t.invoice_qr,t.invoiceQr,m.qr_svg_or_url,t.qr),reviewQr:d(e.reviewQr,t.review_qr,t.reviewQr),notes:d(t.print_note,t.note),simpleBilling:n.simpleBilling,billingLabel:n.billingLabel}},te=(t,e={})=>{const o=e.paperSize||"80mm",n=_t(o),s=o==="58mm",r=e.agentPdf===!0,i=e.customPrintInvoice===!0,a=e.hideInvoiceQr===!0,p=Array.isArray(t.items)?t.items:[],l=Array.isArray(t.kotCodes)?t.kotCodes:[],m=Array.isArray(t.batchCodes)?t.batchCodes:[],_=ot(),$=!(t.simpleBilling??_.simpleBilling),A=i?gt(t.shopName):t.shopName,k=i?ft(t.shopAddress):t.shopAddress,N=d(t.invoiceNo),x=i?s?"48px":"64px":n.paychatLogoWidth,S=a?"":Xt(t.invoiceQr||t.reviewQr),D=t.invoiceUrl&&(a||!S)?`<div class="qr-url">${c(t.invoiceUrl)}</div>`:"";return`<!doctype html>
<html>
<head>
  <meta charset="utf-8">
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .receipt {
      width: ${n.width};
      padding: ${i?"2px 4px 56px":n.padding};
    }
...
<body>
  <div class="receipt">
    ...
    <div class="center">
      ...
      ${k?`<div class="muted">${c(k)}</div>`:""}
      ${t.shopPhone?`<div class="muted">Phone: ${c(t.shopPhone)}</div>`:""}
    </div>
    <div class="line"></div>
    ${N&&!i?`<div class="bill-no">INVOICE NO: ${c(N)}</div>`:""}
    <table>
	      ${N&&i?`<tr class="bill-no-row"><td><strong>Invoice No</strong></td><td class="right"><strong>${c(N)}</strong></td></tr>`:""}
      <tr><td>Date</td><td class="right">${c(bt(t.dateTime))}</td></tr>
      ${t.orderType?`<tr><td>Type</td><td class="right">${c(t.orderType)}</td></tr>`:""}
	      ${($||i)&&t.tableName?`<tr><td>Table</td><td class="right">${c(t.tableName)}</td></tr>`:""}
	      ${$&&t.guestCount&&!i?`<tr><td>Guests</td><td class="right">${c(t.guestCount)}</td></tr>`:""}
	      ${$&&t.tokenNo&&!i?`<tr><td>Token</td><td class="right">${c(t.tokenNo)}</td></tr>`:""}
	      ${$&&l.length?`<tr><td>KOT</td><td class="right">${c(l.join(", "))}</td></tr>`:""}
	      ${$&&m.length?`<tr><td>Batch</td><td class="right">${c(m.join(", "))}</td></tr>`:""}
    </table>
    <div class="line"></div>
    ${s?`
      <div>
        ${p.length?p.map(v=>`
          <div class="item-block">
            <div class="item-name">${c(v.name)}</div>
            <div class="item-meta">
              <span>${c(W(v.qty))} x ${c(u(v.rate))}</span>
              <strong>${c(u(v.total))}</strong>
            </div>
          </div>
    ...
    `:`
      <table>
        ...
        <tbody>
          ${p.length?p.map(v=>`
            <tr>
              <td class="item-name">${c(v.name)}</td>
              <td class="right">${c(W(v.qty))}</td>
              <td class="right">${c(u(v.rate))}</td>
              <td class="right">${c(u(v.total))}</td>
            </tr>
