...
`},H=(t,e)=>{const o=M(t).split(/\s+/).filter(Boolean).flatMap(r=>r.length<=e?[r]:r.match(new RegExp(`.{1,${e}}`,"g"))||[r]),n=[];let s="";return o.forEach(r=>{if(!s){s=r;return}(s+" "+r).length<=e?s+=` ${r}`:(n.push(s),s=r.slice(0,e))}),s&&n.push(s),n.length?n:[""]},ee=(t,e)=>{const o=H(t.name,e),n=`${W(t.qty)} x ${d(t.rate)}`;return[...o.map(s=>`${s}
`),g(n,d(t.total),e)].join("")},ne=(t,e)=>{const r=e-5-9-10,i=H(t.name,r),a=`${i[0].padEnd(r)}${W(t.qty).padStart(5)}${d(t.rate).padStart(9)}${d(t.total).padStart(10)}
`,c=i.slice(1).map(u=>`${u}
...
</html>`},be=(t={},e={})=>({text:ge(t,e),html:fe(t),print_mode:e.printMode||"escpos"}),wt="paychat_print_agent_settings",et={enabled:!1,agentUrl:"http://127.0.0.1:8787",token:"",printerName:"",paperSize:"58mm",printMode:"escpos",autoPrintAfterCheckout:!1,customPrintInvoice:!1,hideInvoiceQr:!1},_e=8e3,pt=12e3,ye=["invoice_url","invoiceUrl","review_url","reviewUrl"],$e=(t,e=null)=>{try{return t?JSON.parse(t):e}catch{return e}},at=(t="")=>String(t??"").replace(/\u20b9\s*/g,"Rs. ").replace(/\b(undefined|null|NaN|Infinity)\b/g,"").replace(/[^\x09\x0A\x0D\x20-\x7E]/g,""),xe=t=>t==="80mm"?"80mm":"58mm",ve=t=>t==="pdf"?"pdf":"escpos",k=(t={})=>({...et,...t&&typeof t=="object"?t:{},enabled:!!t?.enabled,agentUrl:String(t?.agentUrl||et.agentUrl).replace(/\/+$/,""),token:String(t?.token||""),printerName:String(t?.printerName||""),paperSize:xe(t?.paperSize),printMode:ve(t?.printMode),autoPrintAfterCheckout:!!t?.autoPrintAfterCheckout,customPrintInvoice:!!t?.customPrintInvoice,hideInvoiceQr:!!t?.hideInvoiceQr}),E=()=>typeof localStorage>"u"?{...et}:k($e(localStorage.getItem(wt),{})),Ne=(t={})=>{const e=k({...E(),...t});try{localStorage.setItem(wt,JSON.stringify(e))}catch{}return e},Z=(t,e="PRINT_AGENT_ERROR",o=null)=>{const n=new Error(t);return n.code=e,o&&(n.cause=o),n},Q=(t,e={},o={})=>{const n=k(e),s=new URL(t,`${n.agentUrl}/`),r={token:n.token,size:n.paperSize,printer_name:n.printerName,copies:1,print_mode:n.printMode,...o};return Object.entries(r).forEach(([i,a])=>{a!=null&&a!==""&&s.searchParams.set(i,String(a))}),s.toString()},F=async(t,e={},o=_e)=>{const n=new AbortController,s=setTimeout(()=>n.abort(),o);try{const r=await fetch(t,{...e,signal:n.signal}),a=(r.headers.get("content-type")||"").includes("application/json")?await r.json().catch(()=>null):await r.text().catch(()=>"");if(!r.ok)throw Z(a?.message||a?.error||"PayChat Print Agent request failed.","PRINT_AGENT_BAD_RESPONSE");return a}catch(r){throw r?.name==="AbortError"?Z("PayChat Print Agent did not respond in time.","PRINT_AGENT_TIMEOUT",r):r?.code?r:Z("PayChat Print Agent is not running on this device.","PRINT_AGENT_UNAVAILABLE",r)}finally{clearTimeout(s)}},Se=(...t)=>{for(const e of t)if(Array.isArray(e)&&e.length)return e;return[]},Te=(t={})=>!t||typeof t!="object"?!1:!!(t.product||t.menu_item||t.product_snapshot||t.product_name||t.item_name||t.menu_item_name||t.name||t.title||t.quantity||t.qty||t.price||t.rate||t.unit_price||t.total||t.line_total||t.amount),nt=(t,e=0,o=new Set)=>{if(!t||e>4)return[];if(Array.isArray(t))return t.some(Te)?t:[];if(typeof t!="object"||o.has(t))return[];o.add(t);const n=["items","order_items","orderItems","line_items","lineItems","cart"];for(const s of n){const r=nt(t[s],e+1,o);if(r.length)return r}for(const s of Object.values(t)){const r=nt(s,e+1,o);if(r.length)return r}return[]},B=(...t)=>{for(const e of t){const o=Number(e);if(Number.isFinite(o))return o}return 0},P=(...t)=>{for(const e of t){const o=at(e).trim();if(o)return o}return""},Y=(t={})=>{const e=t.invoice||t.invoice_data||{},o=t.data||t.order||{};return Se(t.items,t.order_items,t.orderItems,t.line_items,t.lineItems,t.cart,e.items,e.order_items,o.items,o.order_items,o.line_items,nt(t))},J=(t={})=>P(t.product?.name,t.menu_item?.name,t.product_snapshot?.name,t.product_name,t.item_name,t.menu_item_name,t.name,t.title,t.description,"Item"),K=(t={})=>B(t.quantity,t.qty,t.pivot?.quantity,1)||1,V=(t={})=>{const e=K(t),o=P(t.rate,t.price,t.unit_price,t.unitPrice,t.product?.price);if(o!=="")return Number(o||0);const n=P(t.total,t.line_total,t.amount,t.subtotal);return Number(n||0)/e},Et=(t={})=>{const e=P(t.total,t.line_total,t.amount,t.subtotal);return e!==""?Number(e||0):V(t)*K(t)},Ie=(t=[])=>t.map(e=>({...e,product_name:J(e),name:J(e),quantity:K(e),qty:K(e),rate:V(e),price:V(e),total:Et(e)})),ke=(t,e)=>{const o=at(t);if(o.length<=e)return[o];const n=[];for(let s=0;s<o.length;s+=e)n.push(o.slice(s,s+e));return n},we=(t,e)=>{const o=e==="80mm"?48:32;return at(t).split(/\r?\n/).flatMap(n=>ke(n,o)).join(`
`)},Ee=(t={},e="58mm")=>{const o=e==="80mm"?48:32,n=Y(t);return n.length?n.map(s=>{const r=J(s),i=K(s),a=V(s),u=Et(s).toFixed(2),m=`${i} x ${a.toFixed(2)}`,_=Math.max(1,o-m.length-u.length);return`${r}
${m}${" ".repeat(_)}${u}`}).join(`
`):""},Ae=(t,e,o)=>{const n=Y(e);return!n.length||n.some(r=>{const i=J(r);return i&&t.includes(i.slice(0,Math.min(i.length,12)))})?t:`${t}
${Ee(e,o)}`},Pe=(t,e)=>{if(/total/i.test(t))return t;const o=B(e.total,e.grand_total,e.payable_amount,e.totals?.grand_total,Y(e).reduce((n,s)=>{const r=B(s.quantity,s.qty,1)||1,i=B(s.rate,s.price,s.unit_price);return n+B(s.total,s.line_total,s.amount,r*i)},0));return`${t}
TOTAL ${o.toFixed(2)}`},dt=t=>`\x1BE${t}\x1BE\0`,Oe=(t="",e={})=>{const o=P(e.shopName).replace(/[-_]+/g," ").replace(/\s{2,}/g," ").trim();return String(t||"").split(`
`).map(n=>{const s=n.trim();return s&&(o&&s.toLowerCase()===o.toLowerCase()||/^invoice no\b/i.test(s)||/^total\b/i.test(s))?dt(n):n}).join(`
`)},Ce=(t={},e={})=>{for(const o of ye){const n=P(t[o],e[o]);if(n)return n}return P(e.invoiceUrl,e.reviewUrl,t.invoice?.url,t.meta?.invoice?.url)},Le=t=>{try{const e=new URL(t);return e.protocol==="http:"||e.protocol==="https:"}catch{return!1}},At=(t={},e={},o=E())=>{const n=k(o),s=n.paperSize,r={...t||{},items:Ie(Y(t||{}))},i=Xt(r,e||{}),a={paperSize:s,customPrintInvoice:n.customPrintInvoice,hideInvoiceQr:n.hideInvoiceQr,escposCommands:n.printMode==="escpos"};let c=xt(i,a);const u=Zt(i,{...a,agentPdf:n.printMode==="pdf"});typeof c!="string"&&(c=String(c??"")),c=Ae(c,r,s),c=Pe(c,r),c=we(c,s),n.customPrintInvoice&&(c=Oe(c,i)),c.length>pt&&(c=`${c.slice(0,pt)}
--- Receipt truncated ---`),c=c.replace(/\n*$/,`


`);const m=Ce(t,i),_={text:c,html:u,print_mode:n.printMode};return!n.hideInvoiceQr&&m&&Le(m)&&(_.qr={data:m,size:6,error_correction:"M"}),_},qe=async(t=E())=>{const e=k(t);return F(Q("/health",e,{size:void 0,printer_name:void 0,copies:void 0,print_mode:void 0}),{method:"GET"})},ze=async(t=E())=>{const e=k(t),o=await F(Q("/printers",e,{size:void 0,printer_name:void 0,copies:void 0,print_mode:void 0}),{method:"GET"});return Array.isArray(o)?o:Array.isArray(o?.printers)?o.printers:Array.isArray(o?.data)?o.data:[]},Me=async(t=E())=>{const e=k(t);return F(Q("/test-print",e),{method:"POST"})},De=async(t={},e={})=>{const o=k(e.settings||E()),n=At(t,e.context||{},o);return F(Q("/print",o),{method:"POST",headers:{"Content-Type":"application/json",Accept:"application/json"},body:JSON.stringify(n)})},Ue=async(t={},e={})=>{const o=k(e.settings||E()),n=be(t,{paperSize:o.paperSize,printMode:o.printMode,escposCommands:o.printMode==="escpos"});return F(Q("/print",o),{method:"POST",headers:{"Content-Type":"application/json",Accept:"application/json"},body:JSON.stringify(n)})},Qe={getSettings:E,saveSettings:Ne,checkHealth:qe,getPrinters:ze,testPrint:Me,printReceipt:De,printKot:Ue,buildSafeAgentReceiptPayload:At},Fe={createOrder(t){return h.post("/orders",t)},diningStructure(t={}){return h.get("/dining-structure",{params:t})},bulkSaveTables(t){return h.post("/dining-structure/tables/bulk",t)},updateTablePosition(t,e){return h.patch(`/dining-structure/tables/${t}/position`,e)},list(t={}){return h.get("/tables",{params:t})},update(t,e){return h.patch(`/tables/${t}`,e)},updateStatus(t,e){return h.patch(`/tables/${t}/status`,e)},release(t,e={}){return h.post(`/tables/${t}/release`,e)},createSession(t){return h.post("/table-sessions",t)},openSessions(t={}){return h.get("/table-sessions/open",{params:t})},closeSession(t,e={}){return h.post(`/table-sessions/${t}/close`,e)},assignOrder(t,e){return h.patch(`/orders/${t}/table`,e)},linkOrderTables(t,e){return h.post(`/orders/${t}/tables/link`,e)},sendToKitchen(t,e={}){return h.post(`/orders/${t}/send-to-kitchen`,e)},printKot(t){return h.post(`/orders/${t}/print-kot`)},reprintKitchenBatch(t){return h.post(`/kitchen-batches/${t}/reprint`)},cancelKitchenBatch(t){return h.post(`/kitchen-batches/${t}/cancel`)},generateInlineToken(t){return h.post(`/orders/${t}/inline-token`)}},Pt="paychat_kitchen_operation_mode",Ot="paychat_generate_inline_kitchen_token",Ct="paychat_inline_kitchen_without_status_management",z={DEDICATED_KDS:"dedicated_kds",INLINE:"inline"},Lt=Object.values(z),O=()=>typeof window>"u"?null:window.localStorage||null,ut=t=>{try{const e=O()?.getItem(t);return e?JSON.parse(e):null}catch{return null}},Re=()=>{const t=ut("tenant_settings")||{},e=ut("tenant_info")||{},o=t?.kitchen?.operation_mode||t?.raw?.kitchen_operation_mode||e?.settings?.kitchen?.operation_mode||e?.settings?.raw?.kitchen_operation_mode||e?.tenant?.settings?.kitchen?.operation_mode||e?.tenant?.settings?.raw?.kitchen_operation_mode;return Lt.includes(o)?o:null},Be=()=>{const e=O()?.getItem(Pt);return Lt.includes(e)?e:Re()||z.DEDICATED_KDS},Ge=t=>{const e=t===z.INLINE?z.INLINE:z.DEDICATED_KDS;return O()?.setItem(Pt,e),e},We=()=>Be()===z.INLINE,He=()=>O()?.getItem(Ot)==="true",Je=t=>{const e=!!t;return O()?.setItem(Ot,e?"true":"false"),e},Ve=()=>O()?.getItem(Ct)==="true",Ye=t=>{const e=!!t;return O()?.setItem(Ct,e?"true":"false"),e};export{z as K,Zt as a,Ke as b,Ve as c,He as d,Ye as e,Je as f,Be as g,We as i,Xt as n,Qe as p,Ge as s,Fe as t};
